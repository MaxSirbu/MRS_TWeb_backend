...
public class UserPlanFavoriteController(IUserPlanFavoriteActions favoritesActions) : ControllerBase
{
    // GET api/userplanfavorite?userId=1
    [HttpGet]
    public async Task<IActionResult> GetByUser([FromQuery] int userId)
        => Ok(await favoritesActions.GetByUserAsync(userId));
    ...
    public async Task<IActionResult> Add([FromQuery] int userId, [FromBody] UserPlanFavoriteDto dto)
        => Ok(await favoritesActions.AddAsync(userId, dto));
